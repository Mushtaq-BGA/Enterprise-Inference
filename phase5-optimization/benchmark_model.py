...
LITELLM_API_BASE = "http://localhost:4000"  # LiteLLM via port-forward (same as helper script)
...
def print_metric_table(title: str, rows, value_header: str = "Value (ms)"):
    """Render a simple ASCII table for metric output."""
    border = "+----------------------+--------------------+"
    print(title)
    print(border)
    header = f"| {'Metric':<20} | {value_header:<18}|"
    print(header)
    print(border)
...
async def main():
    ...
    p99_tpot = percentile(tpot_sorted, 0.99)
    ...
    ttft_rows = [
        ("Median", f"{median_ttft * 1000:.2f}"),
        ("P99", f"{p99_ttft * 1000:.2f}")
    ]
    print()
    print_metric_table("--- Time to First Token (TTFT) ---", ttft_rows)

    tpot_rows = [
        ("Median", f"{median_tpot * 1000:.2f}"),
        ("P99", f"{p99_tpot * 1000:.2f}")
    ]
    print()
    print_metric_table("--- Time per Output Token (TPOT) ---", tpot_rows, value_header="Value (ms/token)")
    print()
    print("--- Throughput ---")
